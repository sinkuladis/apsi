                ...
                <div>
                    ...
                    <v-menu v-if="loggedIn" :ripple="false" offset-y>
                        <template v-slot:activator="{ on }">
                            <v-btn fab text x-small dark class="me-4 grey lighten-2 blue--text text--darken-1"
                                   v-on="on">
                                <v-icon medium>mdi-account</v-icon>
                            </v-btn>
                        </template>
                        <v-list subheader>
                            ...
                            <v-list-item class="grey lighten-2 pb-n12">
                                <v-list-item-title class="blue--text text--darken-1">
                                    Wyłoguj się
                                </v-list-item-title>
                            </v-list-item>
...
<script>
    export default {
        ...
        computed: {
            advertRedirect() {
                return this.loggedIn ? "/submitAdvert" : "/login";
            }
        }
    }
</script>
